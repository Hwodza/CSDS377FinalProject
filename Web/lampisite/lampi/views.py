...
class SenderDetailView(LoginRequiredMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if current_stats:
            chart_context['network_rx'] = [stat.rx_kb for stat in current_stats.network_stats.all()]
            chart_context['network_tx'] = [stat.tx_kb for stat in current_stats.network_stats.all()]
            chart_context['network_labels'] = [stat.iface for stat in current_stats.network_stats.all()]
